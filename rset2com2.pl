...
sub RelCoord{
  ...
  #
  #Get the center-of-mass at absolute coordinate
  #
  $sum[0] += $coord->[$first][0];
  $sum[1] += $coord->[$first][1];
  $sum[2] += $coord->[$first][2];
  if ( $bx ){
    $sum[0] -= rint( $sum[0] / $bx ) * $bx;
    $sum[1] -= rint( $sum[1] / $by ) * $by;
    $sum[2] -= rint( $sum[2] / $bz ) * $bz;
  }
...
while(<STDIN>){
	...
	while(<STDIN>){
	    ...
	    my ($rel,$com) = RelCoord( \@mems, \@coord, $bx, $by, $bz );
	    my @rel = @{$rel};
	    my @com = @{$com};
