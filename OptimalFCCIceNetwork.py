...
hy = sqrt(2)
hz = sqrt(2)
box = (NX*hx, NY*hy, NZ*hz)
coord = []
for ix in range(NX):
    for iy in range(NY):
        for iz in range(NZ):
            coord.append((hx*ix, hy*iy, hz*iz))
            coord.append((hx*(ix+0.5), hy*(iy+0.5), hz*iz))
            coord.append((hx*ix, hy*(iy+0.5), hz*(iz+0.5)))
            coord.append((hx*(ix+0.5), hy*iy, hz*(iz+0.5)))
...
for i in range(len(coord)):
    for j in range(len(coord)):
        ...
        if hx*0.5-0.05 < d[0] < hx*0.5+0.05 and hy*0.5-0.05 < d[1] < hy*0.5+0.05 and -0.05 < d[2] < 0.05:
            ...
            if not net.has_key(j):
                net[j] = set()
            net[j].add(i)
        if -0.05 < d[0] < 0.05 and hy*0.5-0.05 < d[1] < hy*0.5+0.05 and hz*0.5-0.05 < d[2] < hz*0.5+0.05:
            if not net.has_key(i):
                net[i] = set()
            net[i].add(j)
            if not net.has_key(j):
